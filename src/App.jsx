...
import {
  ...
  AlertTriangle, ChevronLeft, Search, ArrowUpDown, BookOpen, Download
} from "lucide-react";

// ── Design tokens ────────────────────────────────────────────────────────────
const M3 = {
  primary:"#6366F1", onPrimary:"#FFFFFF", primaryContainer:"#EEF2FF", onPrimaryContainer:"#4338CA",
  secondary:"#6B7280", onSecondary:"#FFFFFF", secondaryContainer:"#F3F4F6", onSecondaryContainer:"#374151",
  surface:"#FFFFFF", surfaceVariant:"#F3F4F6", onSurface:"#111827", onSurfaceVariant:"#6B7280",
  outline:"#D1D5DB", outlineVariant:"#E5E7EB", background:"#F7F8FA",
  error:"#EF4444", onError:"#FFFFFF", errorContainer:"#FEE2E2", onErrorContainer:"#991B1B",
  success:"#10B981", successContainer:"#D1FAE5",
};
...
const STORES = [
  { id:"mb", label:"Market Basket", icon:ShoppingCart, color:"#6366F1", dark:"#4F46E5" },
  { id:"target", label:"Target", icon:TargetIcon, color:"#DC2626", dark:"#B91C1C" },
  { id:"lowes", label:"Lowe's", icon:Hammer, color:"#2563EB", dark:"#1D4ED8" },
];

// ── Icon lookups (mirrors keys in config.js) ─────────────────────────────────
const SECTION_ICONS = {
...
// Strips a leading emoji + space from config.js labels (e.g. "🥦 Aisle 20 — Produce" → "Aisle 20 — Produce")
const stripEmoji = (label) => label.replace(/^\S+\s/, "");

const inp = {
  width:"100%", padding:"10px 12px", border:"1px solid "+M3.outlineVariant, borderRadius:10,
  fontSize:14, fontFamily:font, background:M3.surface, color:M3.onSurface, boxSizing:"border-box", outline:"none",
};

// ── Timer helpers ─────────────────────────────────────────────────────────────
const extractTimerSeconds = (text) => {
  const t=text.toLowerCase(); let total=0;
  const rm=t.match(/(\d+)\s*(?:to|-)\s*(\d+)\s*minutes?/); if(rm){total+=parseInt(rm[2])*60;}
  const rh=t.match(/(\d+)\s*(?:to|-)\s*(\d+)\s*hours?/); if(rh){total+=parseInt(rh[2])*3600;}
  if(rm||rh) return total||null;
  const hrs=t.match(/(\d+(?:\.\d+)?)\s*hours?/), mins=t.match(/(\d+(?:\.\d+)?)\s*minutes?/), secs=t.match(/(\d+)\s*seconds?/);
  if(hrs) total+=parseFloat(hrs[1])*3600; if(mins) total+=parseFloat(mins[1])*60; if(secs) total+=parseInt(secs[1]);
  return total>0?Math.round(total):null;
};
const fmtTime=(s)=>{const h=Math.floor(s/3600),m=Math.floor((s%3600)/60),sec=s%60;if(h>0)return`${h}:${String(m).padStart(2,"0")}:${String(sec).padStart(2,"0")}`;return`${m}:${String(sec).padStart(2,"0")}`;};
...
function StepTimer({seconds,stepText}){
  const[remaining,setRemaining]=useState(seconds),[running,setRunning]=useState(false),[done,setDone]=useState(false),ref=useRef(null);
  useEffect(()=>{setRemaining(seconds);setRunning(false);setDone(false);},[seconds]);
  useEffect(()=>{if(running){reqNotif();ref.current=setInterval(()=>{setRemaining(r=>{if(r<=1){clearInterval(ref.current);setRunning(false);setDone(true);try{sendNotif("Timer done!",stepText?.slice(0,60));}catch(e){}return 0;}return r-1;});},1000);}else clearInterval(ref.current);return()=>clearInterval(ref.current);},[running]);
  const pct=Math.round(((seconds-remaining)/seconds)*100),reset=(e)=>{e.stopPropagation();setRemaining(seconds);setRunning(false);setDone(false);};
  const bg=done?M3.successContainer:running?M3.primaryContainer:M3.surfaceVariant,fg=done?M3.success:M3.primary;
  return(<div onClick={e=>{e.stopPropagation();if(done){reset(e);}else{setRunning(r=>!r);}}} style={{display:"inline-flex",alignItems:"center",gap:8,marginTop:8,background:bg,borderRadius:20,padding:"5px 12px 5px 8px",cursor:"pointer",border:"1px solid "+(done?M3.success:M3.outline)}}><svg width="22" height="22" viewBox="0 0 22 22" style={{flexShrink:0}}><circle cx="11" cy="11" r="9" fill="none" stroke={M3.outlineVariant} strokeWidth="2.5"/>{!done&&<circle cx="11" cy="11" r="9" fill="none" stroke={running?M3.primary:M3.secondary} strokeWidth="2.5" strokeDasharray={`${2*Math.PI*9}`} strokeDashoffset={`${2*Math.PI*9*(1-pct/100)}`} strokeLinecap="round" transform="rotate(-90 11 11)" style={{transition:"stroke-dashoffset 0.9s linear"}}/>}<text x="11" y="15" textAnchor="middle" fontSize="8" fontWeight="700" fill={fg}>{done?"✓":running?"▐▐":"▶"}</text></svg><span style={{fontSize:13,fontWeight:500,color:fg,fontVariantNumeric:"tabular-nums",minWidth:38,textAlign:"center"}}>{done?"Done!":fmtTime(remaining)}</span>{(running||done)&&<span onClick={reset} style={{fontSize:11,color:fg,opacity:0.7}}>↺</span>}</div>);
}
...
function BackBtn({to,label,setView}){
  return(<button onClick={()=>{window.scrollTo(0,0);setView(to);}} style={{background:M3.surface,border:"1px solid "+M3.outlineVariant,borderRadius:20,padding:"6px 14px",cursor:"pointer",display:"flex",alignItems:"center",gap:4,fontFamily:font}}><ChevronLeft size={15} color={M3.onSurface}/><span style={{fontSize:12,fontWeight:500,color:M3.onSurface}}>{label}</span></button>);
}

function GenericItem({item,done,onToggle,onRemove,storeColor}){
  return(<div style={{background:M3.surface,borderRadius:14,border:"1px solid "+M3.outlineVariant,padding:"12px 14px",marginBottom:6,display:"flex",alignItems:"center",gap:10,opacity:done?0.5:1}}><div onClick={()=>onToggle(item.key)} style={{width:20,height:20,borderRadius:"50%",border:"2px solid "+(done?storeColor:M3.outline),background:done?storeColor:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,cursor:"pointer"}}>{done&&<Check size={12} color="white" strokeWidth={3}/>}</div><div onClick={()=>onToggle(item.key)} style={{flex:1,fontSize:14,textDecoration:done?"line-through":"none",color:done?M3.onSurfaceVariant:M3.onSurface,cursor:"pointer"}}>{item.text}</div><span onClick={()=>onRemove(item.key)} style={{cursor:"pointer",padding:"2px 6px",display:"flex"}}><X size={14} color={M3.onSurfaceVariant}/></span></div>);
}

function BottomNav({view,setView,totalItems}){
  const activeTab=(view==="storeSelect"||view==="shopping")?"storeSelect":view==="import"?"import":"recipes";
  const pill=(id)=>({width:64,height:32,borderRadius:16,background:activeTab===id?M3.primaryContainer:"transparent",display:"flex",alignItems:"center",justifyContent:"center",position:"relative"});
  ...
  return(<div style={{position:"fixed",bottom:0,left:0,right:0,background:M3.surface,borderTop:"1px solid "+M3.outlineVariant,display:"flex",zIndex:200,paddingBottom:"env(safe-area-inset-bottom, 0px)"}}><button onClick={()=>setView("storeSelect")} style={{flex:1,background:"none",border:"none",cursor:"pointer",padding:"10px 0 8px",display:"flex",flexDirection:"column",alignItems:"center",gap:3,fontFamily:font,position:"relative"}}><div style={pill("storeSelect")}><ShoppingCart size={20} color={ic("storeSelect")} strokeWidth={1.8}/>{totalItems>0&&<span style={{position:"absolute",top:-2,right:8,background:M3.error,color:M3.onError,borderRadius:"50%",fontSize:9,width:15,height:15,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700}}>{totalItems>99?"99+":totalItems}</span>}</div><span style={label("storeSelect")}>Shopping</span></button><button onClick={()=>setView("recipes")} style={{flex:1,background:"none",border:"none",cursor:"pointer",padding:"10px 0 8px",display:"flex",flexDirection:"column",alignItems:"center",gap:3,fontFamily:font}}><div style={pill("recipes")}><BookOpen size={20} color={ic("recipes")} strokeWidth={1.8}/></div><span style={label("recipes")}>Recipes</span></button><button onClick={()=>setView("import")} style={{flex:1,background:"none",border:"none",cursor:"pointer",padding:"10px 0 8px",display:"flex",flexDirection:"column",alignItems:"center",gap:3,fontFamily:font}}><div style={pill("import")}><Download size={20} color={ic("import")} strokeWidth={1.8}/></div><span style={label("import")}>Import</span></button></div>);
}
...
function MBItem({item, sec, isEd, editingText, setEditingText, editingQty, setEditingQty, onToggle, onEdit, onSaveEdit, onCancelEdit, onRemove, onAislePick, recipes, setSelectedId, setView}){
  ...
  return (
    <div style={{background:M3.surface,borderRadius:14,border:"1px solid "+M3.outlineVariant,padding:"12px 14px",marginBottom:6,display:"flex",alignItems:"flex-start",gap:10}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        {isEd
          ? <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <input type="number" min={1} max={99} autoFocus value={editingQty} onChange={e=>setEditingQty(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")onSaveEdit(item.key);if(e.key==="Escape")onCancelEdit();}} style={{width:50,fontSize:14,padding:"5px 6px",border:"1.5px solid "+M3.primary,borderRadius:8,fontFamily:font,outline:"none",boxSizing:"border-box",background:M3.surface,color:M3.onSurface,textAlign:"center"}}/>
              <input value={editingText} onChange={e=>setEditingText(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")onSaveEdit(item.key);if(e.key==="Escape")onCancelEdit();}} onBlur={()=>onSaveEdit(item.key)} placeholder="item name" style={{flex:1,fontSize:14,padding:"5px 8px",border:"1.5px solid "+M3.primary,borderRadius:8,fontFamily:font,outline:"none",boxSizing:"border-box",background:M3.surface,color:M3.onSurface}}/>
            </div>
          : <div>
      ...
      <span onClick={e=>onEdit(item.key,item.text,e)} style={{cursor:"pointer",padding:"2px 5px",flexShrink:0,display:"flex"}}><Pencil size={13} color={M3.onSurfaceVariant}/></span>
      <span onClick={()=>onRemove(item.key)} style={{cursor:"pointer",padding:"2px 5px",flexShrink:0,display:"flex"}}><X size={13} color={M3.onSurfaceVariant}/></span>
      <span
        onClick={()=>onAislePick(item)}
        title="Click to change aisle"
        style={{fontSize:10,fontWeight:600,color:M3.onPrimaryContainer,background:M3.primaryContainer,borderRadius:999,padding:"3px 9px",whiteSpace:"nowrap",flexShrink:0,cursor:"pointer",userSelect:"none"}}
      >
        A{sec.aisle}
      </span>
    </div>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const run=async()=>{
      ...
      snap.docs.forEach(d=>{const data=d.data();if(data.notes===undefined||data.storage===undefined){batch.update(doc(db,"recipes",d.id),{notes:data.notes??"",storage:data.storage??""});need=true;}});
  ...
  useEffect(()=>{
    ...
    const unsub=onSnapshot(doc(db,"app","mbSettings"),(snap)=>{
      if(snap.exists()){
        const data=snap.data();
        if(data.classOverrides) setClassOverrides(data.classOverrides);
        if(data.itemOrder) setItemOrder(data.itemOrder);
      }
    });
    return()=>unsub();
  },[authReady]);
      ...
      if(snap.exists()){
        ...
        if(data.resetAt&&!targetTimerRef.current){
          ...
          if(rem<=0){resetTarget();}else{
            ...
            targetTimerRef.current=setInterval(()=>{const r=data.resetAt-Date.now();if(r<=0){resetTarget();return;}const m=Math.floor(r/60000),s=Math.floor((r%60000)/1000);setTargetCountdown(`↺ Auto-reset in ${m}:${String(s).padStart(2,"0")}`);},1000);
          }
        }
      }
    });
      ...
      if(snap.exists()){
        ...
        if(data.resetAt&&!lowesTimerRef.current){
          ...
          if(rem<=0){resetLowes();}else{
            ...
            lowesTimerRef.current=setInterval(()=>{const r=data.resetAt-Date.now();if(rem<=0){resetLowes();return;}const m=Math.floor(r/60000),s=Math.floor((r%60000)/1000);setLowesCountdown(`↺ Auto-reset in ${m}:${String(s).padStart(2,"0")}`);},1000);
          }
        }
      }
    });
  ...
  useEffect(()=>{
    ...
    if(tt>0&&tc===tt){
      ...
      targetTimerRef.current=setInterval(()=>{const rem=end-Date.now();if(rem<=0){resetTarget();return;}const m=Math.floor(rem/60000),s=Math.floor((rem%60000)/1000);setTargetCountdown(`↺ Auto-reset in ${m}:${String(s).padStart(2,"0")}`);},1000);
    }else{if(targetTimerRef.current){clearInterval(targetTimerRef.current);targetTimerRef.current=null;}targetResetAtRef.current=null;setTargetCountdown(null);}
  ...
  useEffect(()=>{
    ...
    if(lt>0&&lc===lt){
      ...
      lowesTimerRef.current=setInterval(()=>{const rem=end-Date.now();if(rem<=0){resetLowes();return;}const m=Math.floor(rem/60000),s=Math.floor((rem%60000)/1000);setLowesCountdown(`↺ Auto-reset in ${m}:${String(s).padStart(2,"0")}`);},1000);
    }else{if(lowesTimerRef.current){clearInterval(lowesTimerRef.current);lowesTimerRef.current=null;}lowesResetAtRef.current=null;setLowesCountdown(null);}
  },[lowesChecked,lowesItems]);

  // ── Classification with overrides ─────────────────────────────────────────
  const classifyWithOverrides = (text) => {
    const lower = text.toLowerCase();
    for (const [keyword, sectionKey] of Object.entries(classOverrides)) {
      if (lower.includes(keyword.toLowerCase())) return sectionKey;
    }
  ...
  const saveMBSettings=(overrides, order)=>setDoc(doc(db,"app","mbSettings"),{classOverrides:overrides, itemOrder:order},{merge:true});
    ...
    recipes.filter(r=>checkedIds.has(r.id)).forEach(r=>{
      ...
      r.ingredients.forEach(ing=>{
        ...
        if(lower.includes("salt and pepper")||lower.includes("salt & pepper")){
          ["salt","pepper"].forEach(item=>{const key=`r${r.id}::${item}`;if(!removedKeys.has(key))items.push({key,text:item,recipe:r.title,sectionKey:classifyWithOverrides(item),manual:false});});
        }else{
          const key=`r${r.id}::${ing}`;if(!removedKeys.has(key))items.push({key,text:toShoppingText(scaled),recipe:r.title,sectionKey:classifyWithOverrides(ing),manual:false});
        }
      });
    });
    manualItems.forEach(m=>{if(!removedKeys.has(m.key))items.push({...m,sectionKey:classifyWithOverrides(m.text),manual:true});});
    return items;
  },[checkedIds,recipes,manualItems,removedKeys,classOverrides]);
  ...
  const grouped=useMemo(()=>{
    ...
    Object.keys(g).forEach(k=>{
      ...
      if(itemOrder[k]&&itemOrder[k].length>0){
        const orderMap={};
        itemOrder[k].forEach((key,idx)=>orderMap[key]=idx);
        items=[...items].sort((a,b)=>{
          const ai=orderMap[a.key]??999, bi=orderMap[b.key]??999;
          return ai-bi;
        });
      } else if(k==="produce"){
        items=produceSubSort(items);
      }
      c[k]=items;
    });
    return c;
  },[allShoppingItems,itemOrder]);
  ...
  const handlePhotoImport=async(e)=>{const file=e.target.files?.[0];if(!file)return;setPhotoImporting(true);setPhotoError(null);setShowPhotoImport(true);try{const base64=await new Promise((res,rej)=>{const r=new FileReader();r.onload=()=>res(r.result.split(",")[1]);r.onerror=()=>rej(new Error("Failed to read file"));r.readAsDataURL(file);});const mediaType=file.type||"image/jpeg";const response=await fetch("/api/import-photo",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({base64,mediaType})});const data=await response.json();if(!response.ok)throw new Error(data.error||`API error: ${response.status}`);const recipeText=data.recipeText;if(!recipeText)throw new Error("No recipe text returned");const nr=parseRecipes(recipeText);if(!nr.length)throw new Error("Could not parse recipe from image");await saveRecipesToFirestore(nr);setShowPhotoImport(false);setPhotoImporting(false);if(photoInputRef.current)photoInputRef.current.value="";}catch(err){setPhotoError(err.message||"Something went wrong.");setPhotoImporting(false);}};
  const handleUrlImport=async()=>{if(!urlImportValue.trim())return;setUrlImporting(true);setUrlImportError(null);try{const response=await fetch("/api/import-url",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:urlImportValue.trim()})});const data=await response.json();if(!response.ok)throw new Error(data.error||`API error: ${response.status}`);const recipeText=data.recipeText;if(!recipeText)throw new Error("No recipe text returned");const nr=parseRecipes(recipeText);if(!nr.length)throw new Error("Could not parse a recipe from that URL. Try the text import instead.");await saveRecipesToFirestore(nr);setShowUrlImport(false);setUrlImportValue("");setUrlImporting(false);}catch(err){setUrlImportError(err.message||"Something went wrong.");setUrlImporting(false);}};
  ...
  if(loading)return(
    <div style={{fontFamily:font,background:M3.background,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{textAlign:"center"}}>
        <div style={{width:48,height:48,borderRadius:12,background:M3.primaryContainer,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px"}}><svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M4 6h16M4 12h16M4 18h10" stroke={M3.primary} strokeWidth="2" strokeLinecap="round"/></svg></div>
  ...
  if(view==="storeSelect")return(
    <div style={{fontFamily:font,background:M3.background,minHeight:"100vh",color:M3.onSurface,paddingBottom:90}}>
      <div style={{background:M3.surface,padding:"18px 16px",borderBottom:"1px solid "+M3.outlineVariant}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
          <div style={{width:32,height:32,borderRadius:9,background:M3.primaryContainer,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><ShoppingCart size={17} color={M3.primary}/></div>
          <div style={{fontSize:18,fontWeight:700,letterSpacing:-0.3,color:M3.onSurface}}>{APP_NAME}</div>
        </div>
        <div style={{fontSize:11,color:M3.onSurfaceVariant,letterSpacing:1,textTransform:"uppercase",marginBottom:3}}>Shopping</div>
        <div style={{fontSize:20,fontWeight:700,color:M3.onSurface}}>Choose a store</div>
      </div>
      <div style={{padding:16,display:"flex",flexDirection:"column",gap:10}}>
        {STORES.map(store=>{
          ...
          return(
            <div key={store.id} onClick={()=>{setStoreTab(store.id);setView("shopping");}} style={{background:M3.surface,border:"1px solid "+M3.outlineVariant,borderRadius:16,padding:"16px 18px",display:"flex",alignItems:"center",gap:14,cursor:"pointer"}}>
              <div style={{width:44,height:44,borderRadius:12,background:store.color+"1A",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><StoreIcon size={21} color={store.color}/></div>
              <div style={{flex:1}}>
                <div style={{fontSize:16,fontWeight:600,color:M3.onSurface}}>{store.label}</div>
              ...
              <ChevronLeft size={16} color={M3.outline} style={{transform:"rotate(180deg)"}}/>
            </div>
          );
        })}
      </div>
  ...
  if(view==="shopping"){
    ...
    const sectionLabel={fontSize:11,fontWeight:600,textTransform:"uppercase",letterSpacing:1,color:M3.onSurfaceVariant,padding:"4px 0 6px",borderBottom:"1px solid "+M3.outlineVariant,marginBottom:6,display:"flex",alignItems:"center",gap:6};

    return(
      <div style={{fontFamily:font,background:M3.background,minHeight:"100vh",color:M3.onSurface,paddingBottom:90}}>
        {aisleEditorItem&&<AisleEditorModal item={aisleEditorItem} onSelect={handleAisleSelect} onClose={()=>setAisleEditorItem(null)}/>}
        <div style={{background:M3.surface,padding:"16px 16px 14px",borderBottom:"1px solid "+M3.outlineVariant}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            <div style={{display:"flex",alignItems:"center",gap:9}}>
              <div style={{width:28,height:28,borderRadius:8,background:M3.primaryContainer,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><ShoppingCart size={15} color={M3.primary}/></div>
              <div style={{fontSize:16,fontWeight:700,letterSpacing:-0.3,color:M3.onSurface}}>{APP_NAME}</div>
            </div>
            <BackBtn to="storeSelect" label="Home" setView={setView}/>
          </div>
          <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between"}}>
            <div>
              <div style={{fontSize:11,color:M3.onSurfaceVariant,letterSpacing:1,textTransform:"uppercase",marginBottom:2}}>{store.label}</div>
              <div style={{fontSize:19,fontWeight:700,color:M3.onSurface}}>Shopping list</div>
            </div>
            <button onClick={resetFn} style={{background:M3.surface,color:M3.onSurfaceVariant,border:"1px solid "+M3.outlineVariant,borderRadius:20,padding:"5px 14px",fontSize:12,cursor:"pointer",fontFamily:font,fontWeight:500}}>Reset</button>
          </div>
          <div style={{marginTop:12,background:M3.surfaceVariant,borderRadius:4,height:6,overflow:"hidden"}}>
            <div style={{background:cPct===100?M3.success:M3.primary,width:cPct+"%",height:6,borderRadius:4,transition:"width 0.4s ease"}}/>
          </div>
          <div style={{display:"flex",justifyContent:"space-between",marginTop:5}}>
            <span style={{fontSize:11,color:M3.onSurfaceVariant}}>{cChecked} of {cTotal} items</span>
            <span style={{fontSize:11,color:M3.onSurfaceVariant}}>{cPct}%{countdown?" · "+countdown:""}</span>
          </div>
        </div>
        <div style={{padding:"12px 14px 6px"}}>
          <div style={{display:"flex",gap:8}}>
            <input type="number" min={1} max={99} value={qtyVal} onChange={e=>setQtyVal(e.target.value)} placeholder="1" style={{width:52,padding:"9px 6px",borderRadius:10,border:"1px solid "+M3.outlineVariant,fontSize:14,fontFamily:font,background:M3.surface,textAlign:"center",color:M3.onSurface}}/>
            <input value={inputVal} onChange={e=>setInputVal(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addFn()} placeholder="Add item…" style={{flex:1,padding:"9px 14px",borderRadius:10,border:"1px solid "+M3.outlineVariant,fontSize:14,fontFamily:font,background:M3.surface,color:M3.onSurface,outline:"none"}}/>
            <button onClick={addFn} style={{background:M3.primary,color:M3.onPrimary,border:"none",borderRadius:10,padding:"9px 18px",fontSize:13,cursor:"pointer",fontWeight:600,fontFamily:font}}>Add</button>
          </div>
          ...
          {storeTab==="mb"&&Object.keys(classOverrides).length>0&&(
            <div style={{fontSize:11,color:M3.secondary,marginTop:4}}>
              {Object.keys(classOverrides).length} aisle override{Object.keys(classOverrides).length!==1?"s":""} saved
              <span onClick={()=>{setClassOverrides({});setItemOrder({});saveMBSettings({},{});}} style={{marginLeft:8,color:M3.error,cursor:"pointer",textDecoration:"underline"}}>clear all</span>
            </div>
          )}
        </div>
          ...
          <div style={{padding:"4px 0"}}>
            {activeSections.map(sec=>(
              <div key={sec.key} style={{margin:"10px 14px 0"}}>
                <div style={sectionLabel}>{(()=>{const Ic=SECTION_ICONS[sec.key]||Package;return <Ic size={13}/>;})()}{stripEmoji(sec.label)}</div>
            ...
            {completedItems.length>0&&(
              <div style={{margin:"16px 14px 0"}}>
                <div style={{fontSize:11,fontWeight:600,textTransform:"uppercase",letterSpacing:1,color:M3.onSurfaceVariant,padding:"4px 0 6px",borderBottom:"1px solid "+M3.outlineVariant,marginBottom:6,display:"flex",alignItems:"center",gap:6}}>
                  <Check size={13} color={M3.success} strokeWidth={2.5}/>
                ...
                {completedItems.map(item=>{
                  ...
                  const rnames=item.recipeList||[item.recipe];
                  return(
                    <div key={item.key} style={{background:M3.surface,borderRadius:14,border:"1px solid "+M3.outlineVariant,padding:"12px 14px",marginBottom:6,display:"flex",alignItems:"flex-start",gap:10,opacity:0.6}}>
                      <div onClick={()=>toggleItem(item.key)} style={{width:20,height:20,borderRadius:"50%",border:"2px solid "+M3.primary,background:M3.primary,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,cursor:"pointer",marginTop:2}}><Check size={12} color={M3.onPrimary} strokeWidth={3}/></div>
                      <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{display:"flex",gap:4,flexShrink:0,alignItems:"center"}}>
                        <input type="number" min={1} max={99} placeholder="Qty" value={editingQty} onChange={e=>setEditingQty(e.target.value)} style={{width:40,padding:"4px 6px",borderRadius:6,border:"1px solid "+M3.outlineVariant,fontSize:12,fontFamily:font,background:M3.surface,color:M3.onSurface,textAlign:"center"}}/>
                        <span onClick={()=>removeItem(item.key)} style={{cursor:"pointer",padding:"2px 5px",display:"flex"}}><X size={13} color={M3.onSurfaceVariant}/></span>
                      </div>
                      {sec&&<span style={{fontSize:10,fontWeight:600,color:M3.onSurfaceVariant,background:M3.surfaceVariant,borderRadius:999,padding:"3px 9px",whiteSpace:"nowrap",flexShrink:0}}>A{sec.aisle}</span>}
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{padding:"4px 0"}}>
            {targetActiveCats.map(cat=>(
              <div key={cat.key} style={{margin:"10px 14px 0"}}>
                <div style={sectionLabel}>{(()=>{const Ic=TARGET_ICONS[cat.key]||Package;return <Ic size={13}/>;})()}{stripEmoji(cat.label)}</div>
        ...
        {storeTab==="lowes"&&lowesTotal>0&&(
          <div style={{padding:"4px 0"}}>
            {lowesActiveCats.map(cat=>(
              <div key={cat.key} style={{margin:"10px 14px 0"}}>
                <div style={sectionLabel}>{(()=>{const Ic=LOWES_ICONS[cat.key]||Package;return <Ic size={13}/>;})()}{stripEmoji(cat.label)}</div>
  ...
  if(view==="import")return(
    <div style={{fontFamily:font,background:M3.background,minHeight:"100vh",color:M3.onSurface,paddingBottom:90}}>
      <div style={{background:M3.surface,padding:"18px 16px",borderBottom:"1px solid "+M3.outlineVariant}}>
        ...
        <div style={{fontSize:11,color:M3.onSurfaceVariant,letterSpacing:1,textTransform:"uppercase",marginBottom:3}}>Add recipes</div>
        <div style={{fontSize:19,fontWeight:700,color:M3.onSurface}}>Import</div>
      </div>
      <div style={{padding:16,display:"flex",flexDirection:"column",gap:10}}>
        <div style={{background:M3.surface,border:"1px solid "+M3.outlineVariant,borderRadius:16,overflow:"hidden"}}>
          <button onClick={()=>setShowImport(v=>!v)} style={{width:"100%",background:"none",border:"none",cursor:"pointer",padding:"16px 18px",display:"flex",alignItems:"center",gap:14,fontFamily:font,textAlign:"left"}}>
            <div style={{width:44,height:44,borderRadius:12,background:M3.primaryContainer,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><FileText size={20} color={M3.primary}/></div>
            <div style={{flex:1}}><div style={{fontSize:16,fontWeight:600,color:M3.onSurface}}>Paste text</div><div style={{fontSize:13,color:M3.onSurfaceVariant,marginTop:2}}>Copy & paste a recipe from anywhere</div></div>
            <ChevronLeft size={16} color={M3.outline} style={{transform:showImport?"rotate(90deg)":"rotate(-90deg)"}}/>
          </button>
          {showImport&&(<div style={{padding:"0 16px 16px"}}><textarea value={importText} onChange={e=>setImportText(e.target.value)} style={{width:"100%",height:160,border:"1px solid "+M3.outlineVariant,borderRadius:10,padding:10,fontSize:13,fontFamily:font,background:M3.surfaceVariant,color:M3.onSurface,resize:"vertical",boxSizing:"border-box",outline:"none",lineHeight:1.6}} placeholder={"Recipe Title\nCategory: Mains\nServings: 4\nIngredients:\n- ingredient\nInstructions:\n1. Step one"}/><div style={{display:"flex",gap:8,marginTop:10,justifyContent:"flex-end"}}><button onClick={()=>{setShowImport(false);setImportText("");}} style={{padding:"8px 16px",background:"transparent",color:M3.onSurface,border:"1px solid "+M3.outline,borderRadius:20,cursor:"pointer",fontSize:13,fontFamily:font}}>Clear</button><button onClick={async()=>{await handleImport();setView("recipes");}} style={{padding:"8px 20px",background:M3.primary,color:M3.onPrimary,border:"none",borderRadius:20,cursor:"pointer",fontSize:13,fontFamily:font,fontWeight:600}}>Save recipe</button></div></div>)}
        </div>
        {isMobile&&(<><input ref={photoInputRef} type="file" accept="image/*" capture="environment" onChange={handlePhotoImport} style={{display:"none"}}/><div style={{background:M3.surface,border:"1px solid "+M3.outlineVariant,borderRadius:16,overflow:"hidden"}}><button onClick={()=>photoInputRef.current?.click()} style={{width:"100%",background:"none",border:"none",cursor:"pointer",padding:"16px 18px",display:"flex",alignItems:"center",gap:14,fontFamily:font,textAlign:"left"}}><div style={{width:44,height:44,borderRadius:12,background:M3.primaryContainer,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Camera size={20} color={M3.primary}/></div><div style={{flex:1}}><div style={{fontSize:16,fontWeight:600,color:M3.onSurface}}>Photo</div><div style={{fontSize:13,color:M3.onSurfaceVariant,marginTop:2}}>Take or upload a photo of a recipe</div></div><ChevronLeft size={16} color={M3.outline} style={{transform:"rotate(-90deg)"}}/></button>{showPhotoImport&&(<div style={{padding:"0 16px 16px",textAlign:"center"}}>{photoImporting?(<><div style={{fontSize:13,color:M3.onSurfaceVariant,fontWeight:500}}>Reading recipe from photo…</div><div style={{fontSize:12,color:M3.secondary,marginTop:4}}>This usually takes 5–10 seconds</div></>):photoError?(<><p style={{fontSize:13,color:M3.error,marginBottom:10}}>{photoError}</p><button onClick={()=>{setShowPhotoImport(false);setPhotoError(null);}} style={{padding:"8px 20px",background:M3.primary,color:M3.onPrimary,border:"none",borderRadius:20,cursor:"pointer",fontSize:13,fontFamily:font}}>OK</button></>):null}</div>)}</div></>)}
        <div style={{background:M3.surface,border:"1px solid "+M3.outlineVariant,borderRadius:16,overflow:"hidden"}}>
          <button onClick={()=>setShowUrlImport(v=>!v)} style={{width:"100%",background:"none",border:"none",cursor:"pointer",padding:"16px 18px",display:"flex",alignItems:"center",gap:14,fontFamily:font,textAlign:"left"}}>
            <div style={{width:44,height:44,borderRadius:12,background:M3.primaryContainer,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Link2 size={20} color={M3.primary}/></div>
            <div style={{flex:1}}><div style={{fontSize:16,fontWeight:600,color:M3.onSurface}}>URL / link</div><div style={{fontSize:13,color:M3.onSurfaceVariant,marginTop:2}}>Import directly from a recipe website</div></div>
            <ChevronLeft size={16} color={M3.outline} style={{transform:showUrlImport?"rotate(90deg)":"rotate(-90deg)"}}/>
          </button>
          {showUrlImport&&(<div style={{padding:"0 16px 16px"}}>{urlImporting?(<div style={{textAlign:"center",padding:"8px 0"}}><div style={{fontSize:13,color:M3.onSurfaceVariant,fontWeight:500}}>Fetching recipe…</div><div style={{fontSize:12,color:M3.secondary,marginTop:4}}>This usually takes 5–15 seconds</div></div>):(<>{urlImportError&&<p style={{fontSize:12,color:M3.error,marginBottom:8,display:"flex",alignItems:"center",gap:5}}><AlertTriangle size={13}/> {urlImportError}</p>}<input value={urlImportValue} onChange={e=>setUrlImportValue(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleUrlImport()} placeholder="https://example.com/recipes/…" style={{width:"100%",padding:"10px 12px",border:"1px solid "+M3.outlineVariant,borderRadius:10,fontSize:13,fontFamily:font,background:M3.surfaceVariant,color:M3.onSurface,boxSizing:"border-box",outline:"none"}}/><div style={{display:"flex",gap:8,marginTop:10,justifyContent:"flex-end"}}><button onClick={()=>{setShowUrlImport(false);setUrlImportValue("");setUrlImportError(null);}} style={{padding:"8px 16px",background:"transparent",color:M3.onSurface,border:"1px solid "+M3.outline,borderRadius:20,cursor:"pointer",fontSize:13,fontFamily:font}}>Cancel</button><button onClick={async()=>{await handleUrlImport();}} style={{padding:"8px 20px",background:M3.primary,color:M3.onPrimary,border:"none",borderRadius:20,cursor:"pointer",fontSize:13,fontFamily:font,fontWeight:600}}>Import</button></div></>)}</div>)}
        </div>
      </div>
      <BottomNav view={view} setView={setView} totalItems={totalItems}/>
    </div>
  ...
  if(selected){
    ...
    return(
      <div style={{fontFamily:font,background:M3.background,minHeight:"100vh",color:M3.onSurface,paddingBottom:90}}>
        <div style={{background:M3.surface,padding:"16px 16px 18px",borderBottom:"1px solid "+M3.outlineVariant}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            <button onClick={()=>{setSelectedId(null);cancelEditRecipe();setConfirmDelete(false);if(wakeLock)wakeLock.release();setWakeActive(false);setWakeLock(null);}} style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex",alignItems:"center",gap:4}}><ChevronLeft size={17} color={M3.onSurface}/><span style={{fontSize:13,color:M3.onSurface}}>Back</span></button>
            {!isEditing&&(<div style={{display:"flex",gap:8,alignItems:"center"}}>{confirmDelete?(<><span style={{fontSize:12,color:M3.error}}>Delete?</span><button onClick={()=>deleteRecipe(selected.id)} style={{padding:"5px 12px",background:M3.error,border:"none",borderRadius:20,color:M3.onError,fontSize:12,cursor:"pointer",fontFamily:font}}>Delete</button><button onClick={()=>setConfirmDelete(false)} style={{padding:"5px 12px",background:"transparent",border:"1px solid "+M3.outlineVariant,borderRadius:20,color:M3.onSurface,fontSize:12,cursor:"pointer",fontFamily:font}}>Cancel</button></>):(<><button onClick={()=>setConfirmDelete(true)} style={{padding:"5px 12px",background:"transparent",border:"1px solid "+M3.errorContainer,borderRadius:20,color:M3.error,fontSize:12,cursor:"pointer",fontFamily:font}}>Delete</button><button onClick={()=>startEditRecipe(selected)} style={{padding:"5px 14px",background:M3.primaryContainer,border:"none",borderRadius:20,color:M3.onPrimaryContainer,fontSize:12,cursor:"pointer",fontFamily:font,fontWeight:600}}>Edit</button></>)}</div>)}
            {isEditing&&(<div style={{display:"flex",gap:8}}><button onClick={cancelEditRecipe} style={{padding:"5px 12px",background:"transparent",border:"1px solid "+M3.outlineVariant,borderRadius:20,color:M3.onSurface,fontSize:12,cursor:"pointer",fontFamily:font}}>Cancel</button><button onClick={()=>saveEditRecipe(selected.id)} style={{padding:"5px 14px",background:M3.primaryContainer,border:"none",borderRadius:20,color:M3.onPrimaryContainer,fontSize:12,cursor:"pointer",fontFamily:font,fontWeight:600}}>Save</button></div>)}
          </div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div><div style={{fontSize:21,fontWeight:700,color:M3.onSurface,lineHeight:1.2,marginBottom:3}}>{selected.title}</div><div style={{fontSize:13,color:M3.onSurfaceVariant}}>{selected.category}</div></div>
            <div style={{display:"flex",gap:8,alignItems:"center",marginTop:2}}>
              <button onClick={toggleWakeLock} style={{background:wakeActive?M3.primaryContainer:"transparent",border:"1px solid "+(wakeActive?M3.primaryContainer:M3.outlineVariant),borderRadius:20,padding:"4px 10px",fontSize:11,fontWeight:500,color:wakeActive?M3.onPrimaryContainer:M3.onSurfaceVariant,cursor:"pointer",fontFamily:font,display:"flex",alignItems:"center",gap:4}}><Sun size={12}/> {wakeActive?"On":"Off"}</button>
              <button style={{background:"none",border:"none",cursor:"pointer",display:"flex"}} onClick={()=>toggleFav(selected.id)}><Star size={19} color={M3.primary} fill={selected.favorite?M3.primary:"none"}/></button>
            </div>
          </div>
        </div>
        <div style={{padding:16}}>
          {isEditing?(
            <>
              ...
              <div style={{marginBottom:14}}><label style={{fontSize:11,fontWeight:500,textTransform:"uppercase",letterSpacing:1.2,color:M3.secondary,display:"block",marginBottom:6}}>Servings</label><div style={{display:"flex",alignItems:"center",gap:10}}><button onClick={()=>setEditDraft(d=>({...d,servings:Math.max(1,(parseInt(d.servings)||1)-1)}))} style={{width:34,height:34,borderRadius:"50%",border:"1.5px solid "+M3.outline,background:"transparent",color:M3.primary,fontSize:18,cursor:"pointer",fontFamily:font,display:"flex",alignItems:"center",justifyContent:"center"}}>−</button><input type="number" min={1} max={999} value={editDraft.servings} onChange={e=>setEditDraft(d=>({...d,servings:e.target.value}))} onBlur={()=>setEditDraft(d=>({...d,servings:Math.max(1,parseInt(d.servings)||1)}))} style={{...inp,width:70,textAlign:"center",padding:"8px"}}/><button onClick={()=>setEditDraft(d=>({...d,servings:(parseInt(d.servings)||1)+1}))} style={{width:34,height:34,borderRadius:"50%",background:M3.primary,border:"none",color:M3.onPrimary,fontSize:18,cursor:"pointer",fontFamily:font,display:"flex",alignItems:"center",justifyContent:"center"}}>+</button><span style={{fontSize:12,color:M3.onSurfaceVariant}}>Base: {editDraft.baseServings}</span></div></div>
  ...
  return(
    <div style={{fontFamily:font,background:M3.background,minHeight:"100vh",color:M3.onSurface,paddingBottom:90}}>
      <div style={{background:M3.surface,padding:"16px 16px 0",position:"sticky",top:0,zIndex:100,borderBottom:"1px solid "+M3.outlineVariant}}>
        ...
        <div style={{fontSize:11,color:M3.onSurfaceVariant,letterSpacing:1,textTransform:"uppercase",margin:"10px 0 8px"}}>Recipes</div>
        <div style={{display:"flex",overflowX:"auto",scrollbarWidth:"none",gap:6,paddingBottom:10}}>
          {CATEGORIES.map(cat=>{const active=activeTab===cat;return(<button key={cat} onClick={()=>setActiveTab(cat)} style={{padding:"6px 14px",fontSize:12,cursor:"pointer",whiteSpace:"nowrap",borderRadius:20,fontFamily:font,fontWeight:active?600:400,background:active?M3.primary:"transparent",color:active?M3.onPrimary:M3.onSurfaceVariant,flexShrink:0,border:active?"none":"1px solid "+M3.outlineVariant}}>{cat}</button>);})}
        </div>
      </div>
      <div style={{padding:"12px 14px 6px",display:"flex",gap:8,alignItems:"center"}}>
        <div style={{flex:1,position:"relative",display:"flex",alignItems:"center"}}>
          <Search size={15} color={M3.onSurfaceVariant} style={{position:"absolute",left:12,flexShrink:0,pointerEvents:"none"}}/>
        ...
        <button onClick={()=>setSortAZ(v=>!v)} style={{padding:"9px 14px",borderRadius:20,border:"1px solid "+M3.outlineVariant,background:"transparent",color:M3.onSurfaceVariant,fontSize:12,cursor:"pointer",fontFamily:font,whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:4}}><ArrowUpDown size={12}/>{sortAZ?"A→Z":"Z→A"}</button>
      </div>
      <div style={{padding:"8px 14px 0",paddingBottom:90}}>
        ...
        :filtered.map(r=>(
          <div key={r.id} style={{background:M3.surface,border:"1px solid "+(checkedIds.has(r.id)?M3.primary:M3.outlineVariant),borderRadius:14,padding:"12px 14px",marginBottom:8,display:"flex",alignItems:"center",gap:10}}>
            <div onClick={e=>toggleCheck(r.id,e)} style={{width:22,height:22,borderRadius:7,border:"2px solid "+(checkedIds.has(r.id)?M3.primary:M3.outline),background:checkedIds.has(r.id)?M3.primary:"transparent",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>{checkedIds.has(r.id)&&<Check size={13} color={M3.onPrimary} strokeWidth={3}/>}</div>
            <div style={{flex:1,cursor:"pointer",minWidth:0}} onClick={()=>setSelectedId(r.id)}><p style={{fontSize:15,fontWeight:600,margin:0,color:M3.onSurface,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.title}</p><p style={{fontSize:12,color:M3.onSurfaceVariant,marginTop:2}}>{r.category} · {r.servings} servings</p></div>
            <button style={{background:"none",border:"none",cursor:"pointer",flexShrink:0,display:"flex"}} onClick={e=>toggleFav(r.id,e)}><Star size={18} color={M3.primary} fill={r.favorite?M3.primary:"none"}/></button>
          </div>
        ))}
      </div>
      <BottomNav view={view} setView={setView} totalItems={totalItems}/>

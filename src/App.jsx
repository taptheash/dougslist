...
const M3 = {
  ...
  surface:"#FFFBFE", surfaceVariant:"#E7E0EC", onSurface:"#1C1B1F", onSurfaceVariant:"#49454F",
  outline:"#79747E", outlineVariant:"#CAC4D0", background:"#FFFBFE",
  error:"#B3261E", onError:"#FFFFFF", errorContainer:"#F9DEDC", onErrorContainer:"#410E0B",
  success:"#386A20", successContainer:"#C3EFAC",
};
...
const inp = {
  width:"100%", padding:"10px 12px", border:"1px solid "+M3.outlineVariant, borderRadius:8,
...
function BackBtn({to,label,setView}){
  return(<button onClick={()=>{window.scrollTo(0,0);setView(to);}} style={{background:M3.primaryContainer,border:"none",borderRadius:20,padding:"6px 14px",cursor:"pointer",display:"flex",alignItems:"center",gap:5,fontFamily:font}}><svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M9 3L5 7l4 4" stroke={M3.onPrimaryContainer} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg><span style={{fontSize:12,fontWeight:500,color:M3.onPrimaryContainer}}>{label}</span></button>);
}

function GenericItem({item,done,onToggle,onRemove,storeColor}){
  return(<div style={{background:M3.surface,borderRadius:12,border:"0.5px solid "+M3.outlineVariant,padding:"10px 12px",marginBottom:4,display:"flex",alignItems:"center",gap:10,opacity:done?0.5:1}}><div onClick={()=>onToggle(item.key)} style={{width:20,height:20,borderRadius:"50%",border:"2px solid "+(done?storeColor:M3.outline),background:done?storeColor:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,cursor:"pointer"}}>{done&&<svg width="10" height="10" viewBox="0 0 10 10"><polyline points="2,5 4,7 8,3" stroke="white" strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>}</div><div onClick={()=>onToggle(item.key)} style={{flex:1,fontSize:14,textDecoration:done?"line-through":"none",color:done?M3.onSurfaceVariant:M3.onSurface,cursor:"pointer"}}>{item.text}</div><span onClick={()=>onRemove(item.key)} style={{fontSize:13,color:M3.outlineVariant,cursor:"pointer",padding:"2px 6px"}}>✕</span></div>);
}
    ...
    <div style={{fontFamily:font,background:M3.background,minHeight:"100vh",color:M3.onSurface,paddingBottom:90}}>
      ...
      <div style={{padding:16,display:"flex",flexDirection:"column",gap:10}}>
        {STORES.map(store=>{
          ...
          return(
            <div key={store.id} onClick={()=>{setStoreTab(store.id);setView("shopping");}} style={{background:M3.surface,border:"0.5px solid "+M3.outlineVariant,borderRadius:16,padding:"16px 18px",display:"flex",alignItems:"center",gap:14,cursor:"pointer"}}>
      ...
      <div style={{padding:16,display:"flex",flexDirection:"column",gap:10}}>
        <div style={{background:M3.surface,border:"0.5px solid "+M3.outlineVariant,borderRadius:16,overflow:"hidden"}}>
          <button onClick={()=>setShowImport(v=>!v)} style={{width:"100%",background:"none",border:"none",cursor:"pointer",padding:"16px 18px",display:"flex",alignItems:"center",gap:14,fontFamily:font,textAlign:"left"}}>
            ...
            <div style={{flex:1}}><div style={{fontSize:16,fontWeight:500,color:M3.onSurface}}>Paste text</div><div style={{fontSize:13,color:M3.onSurfaceVariant,marginTop:2}}>Copy & paste a recipe from anywhere</div></div>
        ...
        <div style={{background:M3.surface,border:"0.5px solid "+M3.outlineVariant,borderRadius:16,overflow:"hidden"}}>
          <button onClick={()=>setShowUrlImport(v=>!v)} style={{width:"100%",background:"none",border:"none",cursor:"pointer",padding:"16px 18px",display:"flex",alignItems:"center",gap:14,fontFamily:font,textAlign:"left"}}>
            <div style={{width:48,height:48,borderRadius:12,background:M3.secondaryContainer,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:22}}>🔗</div>
            <div style={{flex:1}}><div style={{fontSize:16,fontWeight:500,color:M3.onSurface}}>URL / link</div><div style={{fontSize:13,color:M3.onSurfaceVariant,marginTop:2}}>Import directly from a recipe website</div></div>
          ...
          {showUrlImport&&(<div style={{padding:"0 16px 16px"}}>{urlImporting?(<div style={{textAlign:"center",padding:"8px 0"}}><div style={{fontSize:13,color:M3.onSurfaceVariant,fontWeight:500}}>Fetching recipe…</div><div style={{fontSize:12,color:M3.secondary,marginTop:4}}>This usually takes 5–15 seconds</div></div>):(<>{urlImportError&&<p style={{fontSize:12,color:M3.error,marginBottom:8}}>⚠ {urlImportError}</p>}<input value={urlImportValue} onChange={e=>setUrlImportValue(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleUrlImport()} placeholder="https://example.com/recipes/…" style={{width:"100%",padding:"10px 12px",border:"1px solid "+M3.outlineVariant,borderRadius:8,fontSize:13,fontFamily:font,background:M3.surfaceVariant,color:M3.onSurface,boxSizing:"border-box",outline:"none"}}/><div style={{display:"flex",gap:8,marginTop:10,justifyContent:"flex-end"}}><button onClick={()=>{setShowUrlImport(false);setUrlImportValue("");setUrlImportError(null);}} style={{padding:"8px 16px",background:"transparent",color:M3.onSurface,border:"1px solid "+M3.outline,borderRadius:20,cursor:"pointer",fontSize:13,fontFamily:font}}>Cancel</button><button onClick={async()=>{await handleUrlImport();}} style={{padding:"8px 20px",background:M3.primary,color:M3.onPrimary,border:"none",borderRadius:20,cursor:"pointer",fontSize:13,fontFamily:font,fontWeight:500}}>Import</button></div></>)}</div>)}
        </div>
      </div>
      <BottomNav view={view} setView={setView} totalItems={totalItems}/>
    </div>
      ...
      <div style={{fontFamily:font,background:M3.background,minHeight:"100vh",color:M3.onSurface,paddingBottom:90}}>
        <div style={{background:M3.primary,padding:"14px 16px 18px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            <button onClick={()=>{setSelectedId(null);cancelEditRecipe();setConfirmDelete(false);if(wakeLock)wakeLock.release();setWakeActive(false);setWakeLock(null);}} style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex",alignItems:"center",gap:6}}><svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M11 4L6 9l5 5" stroke={M3.primaryContainer} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg><span style={{fontSize:13,color:M3.primaryContainer}}>← Back</span></button>
            {!isEditing&&(<div style={{display:"flex",gap:8,alignItems:"center"}}>{confirmDelete?(<><span style={{fontSize:12,color:M3.errorContainer}}>Delete?</span><button onClick={()=>deleteRecipe(selected.id)} style={{padding:"5px 12px",background:M3.error,border:"none",borderRadius:20,color:M3.onError,fontSize:12,cursor:"pointer",fontFamily:font}}>Delete</button><button onClick={()=>setConfirmDelete(false)} style={{padding:"5px 12px",background:"transparent",border:"1px solid "+M3.primaryContainer,borderRadius:20,color:M3.primaryContainer,fontSize:12,cursor:"pointer",fontFamily:font}}>Cancel</button></>):(<><button onClick={()=>setConfirmDelete(true)} style={{padding:"5px 12px",background:"transparent",border:"1px solid "+M3.errorContainer,borderRadius:20,color:M3.errorContainer,fontSize:12,cursor:"pointer",fontFamily:font}}>Delete</button><button onClick={()=>startEditRecipe(selected)} style={{padding:"5px 14px",background:M3.primaryContainer,border:"none",borderRadius:20,color:M3.onPrimaryContainer,fontSize:12,cursor:"pointer",fontFamily:font,fontWeight:500}}>Edit</button></>)}</div>)}
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div><div style={{fontSize:22,fontWeight:500,color:M3.onPrimary,lineHeight:1.2,marginBottom:3}}>{selected.title}</div><div style={{fontSize:13,color:M3.onPrimary+"AA"}}>{selected.category}</div></div>
      ...
      <div style={{padding:"8px 14px 0",paddingBottom:90}}>
        ...
        :filtered.map(r=>(
          <div key={r.id} style={{background:M3.surface,border:"0.5px solid "+(checkedIds.has(r.id)?M3.primary:M3.outlineVariant),borderRadius:12,padding:"11px 14px",marginBottom:8,display:"flex",alignItems:"center",gap:10}}>
            <div onClick={e=>toggleCheck(r.id,e)} style={{width:22,height:22,borderRadius:6,border:"2px solid "+(checkedIds.has(r.id)?M3.primary:M3.outline),background:checkedIds.has(r.id)?M3.primary:"transparent",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>{checkedIds.has(r.id)&&<svg width="12" height="12" viewBox="0 0 12 12"><polyline points="2,6 5,9 10,3" stroke={M3.onPrimary} strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>}</div>
            <div style={{flex:1,cursor:"pointer",minWidth:0}} onClick={()=>setSelectedId(r.id)}><p style={{fontSize:15,fontWeight:500,margin:0,color:M3.onSurface,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.title}</p><p style={{fontSize:12,color:M3.onSurfaceVariant,marginTop:2}}>{r.category} · {r.servings} servings</p></div>

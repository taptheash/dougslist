...
export default function App() {
  ...
  const handleSignIn=async()=>{try{await signInWithPopup(auth,googleProvider);}catch(err){if(err.code==="auth/popup-blocked"){await signInWithRedirect(auth,googleProvider);}else{console.error("Sign in error:",err);}}};
  ...
  // ── Auth gate ──────────────────────────────────────────────────────────────
  if(user===undefined)return(<div style={{fontFamily:font,background:M3.background,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{fontSize:16,color:M3.onSurfaceVariant}}>Loading…</div></div>);
  ...
  if(!user)return(
    <div style={{fontFamily:font,background:M3.background,minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}>
      ...
      <button onClick={handleSignIn} style={{padding:"12px 28px",background:M3.primary,color:M3.onPrimary,border:"none",borderRadius:24,fontSize:15,fontWeight:500,cursor:"pointer",fontFamily:font,marginTop:8}}>Sign in with Google</button>
    </div>
  );
